...
public abstract class Entity implements StadisticsFeed, Renderizable{
	...
	public enum EntityLayer{
		...
		public EntityLayer getLower() { return lower; }

		public EntityLayer getUpper() { return upper; }
	...
	public void render(SpriteBatch batch, float globalScale) {
		if(isDead())
			return;
		batch.draw(getActualTexture(),
				getX(globalScale) + (flipHorizontal() ?  getWidth(globalScale) : 0), getY(globalScale),
				0, 0, 
				getWidth(), getHeight(), 
				(flipHorizontal() ? -1 : 1) * globalScale*scale, globalScale*scale,
				0F);
	}
	...
	protected void moveX(Direction direction, float ammount, float deltaTime) {
		...
		if(Math.abs(velocityX)<DEFAULT_MAX_VELOCITY_X)
			velocityX+=accelerationX * (flipHorizontal() ? -1 : 1);
	...
	public List<Stadistic> getStadistics(float globalScale) {
		...
		stadistics.add(new Stadistic(this,"TilPos(x,y)", "("+fmtFlt(pos.x/map.getTileWidth())+","+fmtFlt(pos.y/map.getTileHeight())+")"));
	...
	public void renderHitBox(ShapeRenderer shapeRenderer, float globalScale) {
		...
		shapeRenderer.rect(
				getArea(globalScale).getX(), getArea(globalScale).getY(), 
				getArea(globalScale).getWidth(), getArea(globalScale).getHeight());
		shapeRenderer.end();
	}
	...
	public void renderDirection(ShapeRenderer shapeRenderer, float globalScale) {
		...
		shapeRenderer.begin(ShapeType.Line);
		Vector2 end = new Vector2(getArea(globalScale).x + ((direction == Direction.RIGHT) ? getArea(globalScale).width : 0F), getArea(globalScale).y + getArea(globalScale).height/2);
		
		//linea inferior
		Vector2 start = new Vector2(getArea(globalScale).x + ((direction == Direction.RIGHT) ? 0F : getArea(globalScale).width), getArea(globalScale).y);
	...
	protected boolean canSeeEntity(Entity entity){
		//TODO lanzar un punto desde this Entidad a la entity, si choca con el mapa retornar falso
		return true;
	}
